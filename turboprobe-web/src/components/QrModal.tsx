...
// Telegram Center Icon (crisp vector on white circle)
const TG_LOGO_DATA_URI = `data:image/svg+xml;utf8,${encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
    <circle cx="50" cy="50" r="49" fill="#FFFFFF"/>
    <circle cx="50" cy="50" r="43" fill="#2481CC"/>
    <path fill="#FFFFFF" d="M28 49.5l42-18c2-.9 4 .5 3.3 2.6L66.5 68.5c-.6 2.3-2 3-4.1 1.9L51 62.5l-5.4 5.2c-.7.7-1.1 1-2.2 1l.8-11.4 21-18.9c.9-.8-.2-1.2-1.3-.5L37.8 54.4 26.8 50.8c-2.3-.8-2.3-2.3.4-3.4z"/>
  </svg>`
)}`;

// TurboProbe Key/Shield Center Icon (crisp vector on white circle)
const VPN_LOGO_DATA_URI = `data:image/svg+xml;utf8,${encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
    <circle cx="50" cy="50" r="49" fill="#FFFFFF"/>
    <circle cx="50" cy="50" r="43" fill="#C25E30"/>
...
export const QrModal: React.FC<QrModalProps> = ({ isOpen, onClose, subUrl }) => {
  ...
  useEffect(() => {
    ...
    // Cohesive, clean single-tone vector QR code (1:1 pixel crisp, no scaling distortion)
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {/* Clean White Card with Tasteful Slim Frame & Native 1:1 Vector Rendering */}
            <div className="p-2.5 bg-white rounded-2xl shadow-sm border border-slate-200/80 mb-3 flex items-center justify-center">
              <div ref={qrRef} className="w-[224px] h-[224px] flex items-center justify-center" />
            </div>
