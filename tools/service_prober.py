...
from verification import (measure_tunnel, node_id, POLICY, check_ru_network, write_ru_snapshot)
...
BATCH_SIZE = int(os.environ.get("TP_BATCH_SIZE", "12"))    # Nodes per Xray instance
NUM_XRAY_WORKERS = int(os.environ.get("TP_XRAY_WORKERS", "2"))  # Concurrent Xray processes
...
PROBE_TIMEOUT = 5.0         # Seconds per real tunnel HTTP request (full TLS handshake allowed)
...
def parse_vless_uri(uri: str, tag: str) -> dict:
    try:
        ...
        if not host or not 1 <= port <= 65535:
            return None
        query = canonicalize_query_keys(urllib.parse.parse_qs(parsed.query))
...
def probe_hy2_tuic_batch(slots: list) -> list:
    """Mihomo per-node SOCKS listeners, explicit proxy pinning, shared strict gate."""
    if not slots:
        return []
    mihomo_bin = get_mihomo_binary_path()
    if not mihomo_bin:
        print("[Mihomo] Unavailable: Hy2/TUIC remain unverified; no DNS fallback", flush=True)
        return []
    proxies, listeners, valid_slots, ports = [], [], [], []
    tmp_dir = tempfile.mkdtemp(prefix="turboprobe_mihomo_")
    proc = None
    try:
        for i, (uri, proto) in enumerate(slots):
            try:
                parsed = urllib.parse.urlparse(uri)
                q = canonicalize_query_keys(urllib.parse.parse_qs(parsed.query))
                host, port, port_range = parse_host_and_port(uri)
                if not host or not 1 <= port <= 65535:
                    continue
                name = f"node_{i}"
                insecure = q.get("insecure", q.get("allowInsecure", ["0"]))[0].lower() in ("1", "true")
                px = {"name": name, "server": host, "port": port,
                      "sni": q.get("sni", [host])[0], "skip-cert-verify": insecure}
                scheme = parsed.scheme.lower()
                if scheme in ("hy2", "hysteria2"):
                    px.update(type="hysteria2", password=urllib.parse.unquote(parsed.username or ""))
                    hopping = q.get("ports", q.get("mport", [port_range]))[0]
                    if "-" in hopping or "," in hopping:
                        px["ports"] = hopping
                    obfs = q.get("obfs", [""])[0]
                    if obfs and obfs != "none":
                        px["obfs"] = obfs
                        px["obfs-password"] = q.get("obfs-password", q.get("obfsParam", [""]))[0]
                elif scheme == "tuic":
                    px.update(type="tuic", uuid=urllib.parse.unquote(parsed.username or ""),
                              password=urllib.parse.unquote(parsed.password or q.get("password", [""])[0]))
                    px["congestion-controller"] = q.get("congestion_control", ["bbr"])[0]
                    if q.get("alpn"):
                        px["alpn"] = q["alpn"][0].split(",")
                else:
                    continue
                local_port = allocate_free_socks_port()
                ports.append(local_port)
                proxies.append(px)
                listeners.append({"name": f"in_{i}", "type": "socks", "port": local_port,
                                  "listen": "127.0.0.1", "udp": False, "proxy": name})
                valid_slots.append((uri, scheme, local_port))
            except (ValueError, RuntimeError):
                continue
        if not valid_slots:
            return []
        cfg = {"mode": "rule", "log-level": "silent", "allow-lan": False,
               "proxies": proxies, "listeners": listeners, "rules": ["MATCH,REJECT"]}
        cfg_file = os.path.join(tmp_dir, "config.json")
        with open(cfg_file, "w", encoding="utf-8") as f:
            json.dump(cfg, f)
        check = subprocess.run([mihomo_bin, "-t", "-d", tmp_dir, "-f", cfg_file],
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=15)
        if check.returncode:
            return []
        proc = subprocess.Popen([mihomo_bin, "-d", tmp_dir, "-f", cfg_file],
                                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        missing = set(wait_for_ports_ready(ports, max_wait=MIHOMO_STARTUP_TIMEOUT))
        ...
        def check_slot(slot):
            ...
            if port in missing:
                return None
            result = measure_tunnel(port, timeout=PROBE_TIMEOUT)
            return {**result, "uri": uri, "protocol": proto, "services": {}} if result else None
        with ThreadPoolExecutor(max_workers=min(12, len(valid_slots))) as pool:
            return [r for r in pool.map(check_slot, valid_slots) if r]
    finally:
        if proc:
            proc.kill()
            proc.wait(timeout=5)
...
# =============================================================================
def _client_reachability_ok(session, deadline=None):
    raise RuntimeError("Use measure_tunnel: one HTTP response is not a verified tunnel")


def probe_ipwho_liveness(session, uri):
    raise RuntimeError("Legacy permissive liveness gate removed; use measure_tunnel with an explicit SOCKS port")
...
def probe_node_basic_liveness(port: int, uri: str) -> tuple:
    ...
    return (True, result["country"], result["ping_ms"], result["speed_mbps"], {})


def probe_node_liveness_and_services(port: int, uri: str) -> tuple:
    # Tunnel reachability does not imply access to an AI account, video playback
    # or Discord voice. Leave service flags unknown until service-specific
    # content validators exist; do not treat 403/404 as universal success.
    return probe_node_basic_liveness(port, uri)
...
def run_batch_probe(xray_bin: str, batch: list, base_port: int = BASE_SOCKS_PORT, basic_only: bool = False) -> list:
    ...
    cfg = {
        "log": {"loglevel": "none"},
        "inbounds": inbounds,
        "outbounds": [{"tag": "blocked", "protocol": "blackhole"}] + outbounds,
        "routing": {"rules": rules},
    }
    ...
    def _validate_nodes_individually() -> int:
        ...
        def single_test(slot):
            ...
            single_cfg = {
                ...
                "outbounds": [{"tag": "blocked", "protocol": "blackhole"}] + [item for item in outbounds if item.get("tag") == f"out_{idx}"],
                ...
                try:
                    ...
                    if is_alive:
                        results.append({
                            ...
                            "protocol": proto,
                            "services": services,
                            "verified": True, "verification_policy": POLICY,
                            "ping_kind": "https_tunnel",
...
def deep_verify_nodes(uris: list, batch_size: int = BATCH_SIZE, return_results=False):
    """Fresh full-tunnel checks only. Never reuse cloud results for RU measurement."""
    if not uris:
        return {} if return_results else set()
    ...
    with ThreadPoolExecutor(max_workers=min(NUM_XRAY_WORKERS, len(batches) or 1)) as executor:
        ...
        for future in as_completed(futures):
            for n in future.result():
                if n.get("verified") is True and n.get("verification_policy") == POLICY:
                    verified[node_id(n["uri"])] = n
    return verified if return_results else {n["uri"].split("#", 1)[0] for n in verified.values()}
...
def verify_nodes_with_globalping_ru(nodes: list, max_nodes: int = 40) -> list:
    # ICMP from an unrelated RU probe cannot verify VPN credentials or transport.
    # Retained for API compatibility; only the dedicated RU snapshot is authoritative.
    return nodes
...
def main():
    ...
    ru_vantage = check_ru_network() if args.vantage == "ru-local" else None
    ...
    if not candidates and args.vantage != "ru-local":
    ...
    xray_bin = get_xray_binary_path()
    if not xray_bin:
        raise RuntimeError("Xray binary unavailable; verification failed")
    ...
    if args.vantage == "ru-local":
        # Check the measuring network again after the run, then publish only this
        # run's fresh measurements. Never merge the previous successful pool.
        ru_vantage = check_ru_network()
        write_ru_snapshot(SUB_DIR, verified_alive_nodes, ru_vantage)
        print("Saved strict Intersvyaz snapshot; cloud outputs unchanged", flush=True)
        return
    ...
    for fname in os.listdir(countries_dir):
        if fname.endswith(".txt"):
            open(os.path.join(countries_dir, fname), "w").close()
