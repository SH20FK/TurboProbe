import { handleRu, fresh, POLICY } from './ru.js';
/**
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === "/sub/ru-intersvyaz") return handleRu(request);
  ...
  <div class="meta">TurboProbe Edge &middot; SH20FK &middot; 2026</div>
    ...
    try {
      ...
      if (allNodes.length === 0) {
        const lines = []; // No metadata: cannot establish freshness or verification.
        if (lines.length > 0) {
      ...
      // Empty means empty: never substitute nodes that violate the filters.
      if (matching.length === 0) {
        return new Response("No nodes match the requested filters.", {status: 503, headers: {"Cache-Control": "no-store", "Access-Control-Allow-Origin": "*"}});
      }
...
async function fetchFirstSuccessfulJson(mirrors, timeoutMs = 3500) {
  const promises = mirrors.map(async (mirrorUrl) => {
    ...
    try {
      ...
      const validList = list.filter(n => n && n.verified === true && n.verification_policy === POLICY && fresh(n.checked_at) && typeof n.uri === 'string' && !n.uri.startsWith('<') && !n.uri.startsWith('=') && !n.uri.startsWith('>'));
...
function generateSingboxJson(nodes) {
  ...
  if (outbounds.length === 0) {
    return {outbounds: [{type: 'block', tag: 'block'}], route: {final: 'block'}};
  }
