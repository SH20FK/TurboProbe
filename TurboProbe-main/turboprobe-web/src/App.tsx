...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (res && isMounted) {
          ...
          const mapped: NodeItem[] = lines.map((uri) => {
    ...
    const expire = setInterval(() => setAllNodes(prev => normalizeAndIndexNodes(prev)), 30_000);
  ...
  const measuredStats = useMemo(() => {
    const pings = allNodes.filter(n => n.verified && (n.ping_ms || 0) > 0).map(n => n.ping_ms!);
    return { best: pings.length ? Math.min(...pings) : 0,
      average: pings.length ? Math.round(pings.reduce((sum, ping) => sum + ping, 0) / pings.length) : 0 };
  }, [allNodes]);
          ...
          <div className="w-full max-w-3xl mx-auto space-y-4 px-3 sm:px-4">
            ...
            <div style={{ display: appMode === 'vpn' ? 'block' : 'none' }} className="space-y-4">
              ...
              <Header
                ...
                bestPing={measuredStats.best}
                avgPing={measuredStats.average}
              ...
              <footer className="w-full pt-8 pb-4 border-t border-[var(--border-main)] flex flex-col items-center justify-center text-center text-xs text-[var(--text-muted)] font-body space-y-1">
                ...
                <p className="m-0 font-mono text-[11px]">
                  HTTPS-замер с точки проверки, не с вашего устройства • Срок подтверждения: 1 час
                </p>
              </footer>
            </div>
          </div>
